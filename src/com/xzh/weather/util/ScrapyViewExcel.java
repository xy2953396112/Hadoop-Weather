...
public class ScrapyViewExcel extends AbstractExcelView {
   
	protected void buildExcelDocument(Map<String, Object> model,
            HSSFWorkbook workbook, HttpServletRequest request,
            HttpServletResponse response) throws Exception {
         List<Scrapy> scrapy= (List<Scrapy>) model.get("scrapyToExcel");
         Date now = new Date();
         SimpleDateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd");//可以方便地修改日期格式
         String file = dateFormat.format( now ); 
         response.setHeader("Content-Disposition", "attachment;filename="+file+"_scrapy.xls");
         HSSFSheet sheet = workbook.createSheet("历史天气");
